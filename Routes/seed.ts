...
seedRouter.post("/seed/recurring-expenses", userAuth, async (req: Request, res: Response) => {
  try {
    ...
    logEvent("info", "Recurring test expenses seeded", {
    ...
    logApiError(req, err, { route: "POST /seed/recurring-expenses" });
...
seedRouter.delete("/seed/recurring-expenses", userAuth, async (req: Request, res: Response) => {
  try {
    const userId = (req as any).user?._id;
    ...
    logEvent("info", "Test recurring expenses deleted", {
...
seedRouter.post("/seed/transactions", userAuth, async (req: Request, res: Response) => {
  try {
    const userId = (req as any).user?._id;
    if (!userId) {
      return res.status(401).json({ message: "Not authenticated" });
    ...
    logEvent("info", "Seed transactions created", {
    ...
    logApiError(req, err as Error, { route: "POST /seed/transactions" });
