...
expenseAnalyticsRouter.get("/expenseAnalytics/range", userAuth_1.default, async (req, res) => {
    try {
        ...
        const [startYear, startMonth, startDay] = rawStartDate.split("-").map((part) => Number(part));
        const [endYear, endMonth, endDay] = rawEndDate.split("-").map((part) => Number(part));
        const startDate = new Date(Date.UTC(startYear, startMonth - 1, startDay, 0, 0, 0, 0) + tzOffsetMinutes * 60 * 1000);
        const endDate = new Date(Date.UTC(endYear, endMonth - 1, endDay, 23, 59, 59, 999) + tzOffsetMinutes * 60 * 1000);
...
expenseAnalyticsRouter.get("/expenseAnalytics/recurring", userAuth_1.default, async (req, res) => {
    try {
        ...
        const recurringWithFrequency = recurringData.map((item) => {
            ...
            const confidenceScore = Math.min(100, Math.round((Math.min(item.count, 6) / 6) * 50 + Math.max(0, amountConsistency) * 50));
            return {
                name: item.name,
            ...
            ...(debug && {
                debug: {
                    ...
                    allProcessed: recurringWithFrequency.map((item) => ({
                        ...
                        reason: item.confidenceScore < 40
...
exports.default = expenseAnalyticsRouter;
