...
expressRouter.post("/expense/add", userAuth_1.default, async (req, res, next) => {
    try {
        ...
        const userId = req.user._id; // from userAuth
...
expressRouter.get("/expense/:date/paged", userAuth_1.default, async (req, res) => {
    try {
        const userId = req.user._id;
        const rawDate = req.params.date;
        if (!rawDate || typeof rawDate !== "string" || !/^\d{4}-\d{2}-\d{2}$/.test(rawDate)) {
        ...
        const docs = await ExpenseSchema_1.default.find(baseFilter)
            .sort({ occurredAt: -1 })
            .limit(limit + 1);
